...
pub fn temperature_icon<D>(display: &mut D)
where 
    D:DrawTarget<Color = Rgb565>+Dimensions {

    let icon_data = include_bytes!("../icons/temperature.bmp");
    let temperature = Bmp::from_slice(icon_data).unwrap();
    Image::new(&temperature, Point::new(POS_X + 3, POS_Y)).draw(display);
    
}

pub fn temperature_field<D>(display: &mut D)
where 
    D:DrawTarget<Color = Rgb565>+Dimensions {
...
pub fn update_temperature<D>(display: &mut D, temp_data: f32)
where 
    D:DrawTarget<Color = Rgb565>+Dimensions {
        ...
        write!(data_string,"{:.1}", temp_data).unwrap();
